...
public class AddInternship extends AppCompatActivity {
    ...
    void makerequest(final String name, final String det,final String link){
        URL u = new URL();
        StringRequest s = new StringRequest(Request.Method.POST, u.url+"internship.php", new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                Toast.makeText(getApplicationContext(),response,Toast.LENGTH_SHORT).show();
                if(response.trim().equals("done")){
                    Toast.makeText(getApplicationContext(),"Internship details added",Toast.LENGTH_SHORT).show();
